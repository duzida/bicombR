setGeneric("Resfronts", function(obj, ...) standardGeneric("Resfronts"))
...
setMethod("Resfronts", "ABprofile", function(obj, tdm, threshold_method="auto", 
                                           timewindow = 3, plotnet=F){
  names(obj@MAJR) <- obj@PMID
  term.list <- obj@MAJR[obj@MAJR != "NO Result Found"]
  
  threshold_method <- match.arg(threshold_method, c("auto", "input"))
  
  if(threshold_method == "input"){
    ...
    print(p1)
    minfreq <- as.numeric(readline("Input the minimum Mesh term frequency reserved\n"))
    if(!minfreq %in% 1:max(as.integer(plotly_data(p1)$occurence_frequence))){
      stop("Please input the number with correct range!\n")
    }
  }else{
  ...
  if(nrow(tdm) < 20){
    stop("the number of Mesh term must > 20")
  }
  tdm <-tdm[rowSums(tdm)>=minfreq,]
  tdm <-tdm[, colSums(tdm)!=0]
  m.sim <- lsa::cosine(as.matrix(t(tdm)))
  g <- graph.adjacency(m.sim, mode = "undirected", weighted = T, diag = F)
  
  set.seed(1000)
  wc <- cluster_walktrap(g, steps = 5)#0.49
  deg <- c()
  dens <- c()
  dist <- c()
  centr <- c()
  nov <- c()
  for(i in 1:length(groups(wc))){
    subg <- induced_subgraph(g, match(groups(wc)[[i]], V(g)$name))
    deg[i] <- (sum(degree(g, match(groups(wc)[[i]], V(g)$name)))- sum(degree(subg)))/length(groups(wc)[[i]])
    dens[i] <- edge_density(subg)
    dist[i] <- mean_distance(subg)
    centr[i] <- centr_betw(subg)$centralization
    nov[i] <- mean(Buzzindex(V(subg)$name, timewindow, obj))
  }
  # source("../BY/ggradar.R")
  radarchart.df <- data.frame(group = names(groups(wc)), deg, dens, dist, centr, nov)
  radarchart.df[is.na(as.matrix(radarchart.df))] <- 0
  radarchart.df[,-1] <- sapply(2:6, function(x)sapply(radarchart.df[,x], function(p)(p-min(radarchart.df[,x]))/(max(radarchart.df[,x])--min(radarchart.df[,x]))))
  radarchart.df[is.na(as.matrix(radarchart.df))] <- 0
  write.table(radarchart.df, "hot.txt", sep = "\t", quote = F, col.names = T, row.names = F)
  p1 <- ggradar(radarchart.df)
  print(p1)
  
  if(plotnet==T){
    dimnames(m.sim) <- NULL
    g <- graph.adjacency(m.sim, mode = "undirected", weighted = T, diag = F)
    m.df <- toVisNetworkData(g)
    m.df$nodes$title <- paste0("<p>", rownames(tdm), "<br>ClusterID:", wc$membership, "<br>degree:", degree(g))
    m.df$nodes$label <- NULL
    m.df$nodes$group <- factor(wc$membership)
    p2 <- visNetwork(nodes = m.df$nodes, edges = m.df$edges, height = "1000px", width = "100%") %>% 
